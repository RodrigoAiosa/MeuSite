...
def obter_credenciais():
    """Corrige o erro de assinatura JWT limpando a chave do JSON."""
    ...
    # RESOLUÇÃO DO ERRO JWT: Limpa espaços e formata quebras de linha corretamente
...
def registrar_acesso(nome_pagina, acao="Visualização"):
    """Registra logs e preserva dados existentes [Regra de Ouro]."""
    try:
...
def salvar_formulario_contato(dados):
    """Adiciona nova linha preservando todo o histórico da tabela."""
    try:
        creds = obter_credenciais()
        client = gspread.authorize(creds)
        id_planilha = "1JXVHEK4qjj4CJUdfaapKjBxl_WFmBDFHMJyIItxfchU"
...
def exibir_rodape():
    """Exibe o rodapé unificado SKY DATA SOLUTION."""
    st.markdown(
        """
